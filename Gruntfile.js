module.exports = function (grunt) {
  ...
  grunt.initConfig({
    ...
    hideBasePath: 'App_Plugins/<%= pkg.name %>Hide',

    uglify: {
      dist: {
        files: {
          '<%= dest %>/<%= basePath %>/js/preview.genie.controller.min.js': ['app/scripts/controllers/preview.genie.controller.js']
        }
      }
    },
    ...
    //   dist: {
    //     src: [
    //       'app/scripts/controllers/preview.genie.controller.js'
    //     ],
    //     dest: '<%= dest %>/<%= basePath %>/js/preview.genie.controller.js',
    //     nonull: true
    //   }
    // },
    ...
    less: {
      dist: {
        ...
        files: {
          '<%= dest %>/<%= basePath %>/css/preview.genie.css': 'app/styles/preview.genie.less',
        }
      },
      distHide: {
        options: {
          paths: ['app/hideStyles'],
        },
        files: {
          '<%= dest %>/<%= basePath %>/css/preview.genie.css': 'app/styles/preview.genie.less',
        }
    ...
    watch: {
      ...
      config: {
        files: ['app/package.manifest'],
        tasks: ['copy:config']
      },

      configHide: {
        files: ['app.hide/package.manifest'],
        tasks: ['copy:configHide']
      }
    },
    ...
    copy: {
      ...
      configHide: {
        src: 'app.hide/package.manifest',
        dest: '<%= dest %>/<%= hideBasePath %>/package.manifest',
      },
  ...
  grunt.registerTask('default', ['jshint', 'less', 'uglify', 'copy:config', 'copy:views']);
